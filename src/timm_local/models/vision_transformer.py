...
from timm_local.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD, \
    OPENAI_CLIP_MEAN, OPENAI_CLIP_STD
from timm_local.layers import PatchEmbed, Mlp, DropPath, AttentionPoolLatent, RmsNorm, PatchDropout, SwiGLUPacked, SwiGLU, \
...
def global_pool_nlc(
    ...
    if pool_type == 'token':
        # x = x[:, 0]  # class token
        # NOTE may not need / siglip use 'map' pooler
        if vit_causal_mask:
            x =  x[:, -1]   # last token is the cls token
        else:
            x = x[:, 0]
    else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            vit_causal_mask: bool = True, # vit causal mask
    ) -> None:
        """
        Args:
        ...
        # mlp mapping
        self.vq_table = nn.Linear(codebook_embed_dim, 4 * embed_dim)
        self.vq_act = nn.GELU()
        self.vq_mapping = nn.Linear(4 * embed_dim, embed_dim)

        # causal mask
        self.vit_causal_mask = vit_causal_mask
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.no_embed_class:
            ...
            if to_cat:
                # x = torch.cat(to_cat + [x], dim=1)
                # NOTE not need, no cls token for siglip
                if self.vit_causal_mask:
                    # cat x then to_cat
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # NOTE VQ model
        quant, emb_loss, info = self.vq.encode(x)   # quant: [B, C, H, W]
        quant = quant.reshape(quant.shape[0], quant.shape[1], -1)  # [B, C, H*W]
        quant = quant.permute(0, 2, 1)  # [B, H*W, C]
        quant = quant.to(x.dtype)
        
        x = self.vq_table(quant)
        x = self.vq_act(x)
        x = self.vq_mapping(x)
        ...
        # Attn mask
        N = x.shape[1]
        mask_causal = torch.empty(N, N, device=x.device)
        mask_causal.fill_(float("-inf"))
        mask_causal.triu_(1)  # zero out the lower diagonal
        mask_causal = mask_causal.type_as(x)
    ...
    def pool(self, x: torch.Tensor, pool_type: Optional[str] = None) -> torch.Tensor:
        ...
        x = global_pool_nlc(x, pool_type=pool_type, num_prefix_tokens=self.num_prefix_tokens, vit_causal_mask=self.vit_causal_mask)
        return x
...
default_cfgs = {
    ...
    'vit_so400m_patch14_siglip_224.webli': _cfg(
        ...
        num_classes=0),
    # toklip (siglip)
    'vit_so400m_patch14_toklip_224.webli': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 256, 256),
        num_classes=0),
    ...
    'vit_so400m_patch14_siglip_384.webli': _cfg(
        ...
        num_classes=0),
    # toklip (siglip)
    'vit_so400m_patch14_toklip_384.webli': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 432, 432),
        num_classes=0),
    # siglip2
    'vit_so400m_patch16_siglip_256.v2_webli': _cfg(
        ...
        input_size=(3, 256, 256),
        num_classes=0),
    'vit_so400m_patch16_toklip_256.v2_webli': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 256, 256),
        num_classes=0),
    ...
    'vit_so400m_patch16_siglip_384.v2_webli': _cfg(
        ...
        num_classes=0),
    # toklip (siglip2)
    'vit_so400m_patch16_toklip_384.v2_webli': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 384, 384),
        num_classes=0),
...
@register_model
def vit_so400m_patch14_toklip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(
        patch_size=16, embed_dim=1152, depth=27, num_heads=16, mlp_ratio=3.7362, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'vit_so400m_patch14_toklip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
# toklip
def vit_so400m_patch14_toklip_384(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(
        patch_size=16, embed_dim=1152, depth=27, num_heads=16, mlp_ratio=3.7362, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'vit_so400m_patch14_toklip_384', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_so400m_patch16_toklip_256(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(
        patch_size=16, embed_dim=1152, depth=27, num_heads=16, mlp_ratio=3.7362, class_token=False, global_pool='map',
        act_layer='gelu_tanh',
    )
    model = _create_vision_transformer(
        'vit_so400m_patch16_toklip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
# vit_so400m_patch16_toklip_384
@register_model
def vit_so400m_patch16_toklip_384(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(
        patch_size=16, embed_dim=1152, depth=27, num_heads=16, mlp_ratio=3.7362, class_token=False, global_pool='map',
        act_layer='gelu_tanh',
    )
    model = _create_vision_transformer(
        'vit_so400m_patch16_toklip_384', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
