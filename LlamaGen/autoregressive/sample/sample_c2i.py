...
def main(args):
    ...
    class_labels = [726, 917, 13, 939, 6, 983, 655, 579]
    ...
    folder_name = f"{model_string_name}-{ckpt_string_name}-size-{args.image_size}-{args.vq_model}-" \
                  f"topk-{args.top_k}-topp-{args.top_p}-temperature-{args.temperature}-" \
                  f"cfg-{args.cfg_scale}-seed-{args.seed}"
                  
    sample_folder_dir = f"{args.c2i_sample_dir}/{folder_name}"
    # if rank == 0:
    os.makedirs(sample_folder_dir, exist_ok=True)
    print(f"Saving .png samples at {sample_folder_dir}")
        
    for i, sample in enumerate(samples):
        # Image.fromarray(sample).save(f"{sample_folder_dir}/{class_labels[i]}.png")
        out_path=f"{sample_folder_dir}/{class_labels[i]}.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--c2i-sample-dir", type=str, default="samples-c2i-argmax")
