...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    """
    Builds a single .npz file from a folder of .png samples without keeping all images in RAM.
    Uses a temporary numpy.memmap to stream images to disk, then saves to .npz.
    """
    first_path = f"{sample_dir}/{0:06d}.png"
    first_img = Image.open(first_path).convert("RGB")
    first_np = np.asarray(first_img).astype(np.uint8)
    h, w, c = first_np.shape
    assert c == 3, "Expected RGB images with 3 channels"

    memmap_path = os.path.join(sample_dir, "__tmp_memmap.dat")
    mm = np.memmap(memmap_path, dtype=np.uint8, mode="w+", shape=(num, h, w, 3))
    mm[0] = first_np

    for i in tqdm(range(1, num), desc="Building .npz file from samples"):
        sample_pil = Image.open(f"{sample_dir}/{i:06d}.png").convert("RGB")
        sample_np = np.asarray(sample_pil).astype(np.uint8)
        assert sample_np.shape == (h, w, 3), f"Image {i} shape mismatch: {sample_np.shape} vs {(h, w, 3)}"
        mm[i] = sample_np

    mm.flush()
    npz_path = f"{sample_dir}.npz"
    np.savez(npz_path, arr_0=mm)
    print(f"Saved .npz file to {npz_path} [shape={(num, h, w, 3)}].")
    try:
        del mm  # close memmap
    finally:
        try:
...
def main(args):
    ...
    if rank == 0:
        ...
        # Optionally remove the PNG folder to keep only NPZ
        try:
            import shutil
            shutil.rmtree(sample_folder_dir)
            print(f"Removed intermediate sample folder: {sample_folder_dir}")
        except Exception as e:
            print(f"Warning: failed to remove sample folder {sample_folder_dir}: {e}")
        print(f"Done. Saved NPZ at {npz_path}.")
    dist.barrier()
